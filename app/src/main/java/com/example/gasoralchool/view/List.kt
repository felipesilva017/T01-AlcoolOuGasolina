...
import com.example.gasoralchool.data.Posto
...
fun ListaDePostos(navController: NavHostController, nomeDoPosto: String) {
  ...
  val postos = listOf("$nomeDoPosto", "Outro posto", "Mais um posto")
  val postosComp = listOf(postoN, postoSP, postoNY)
  Scaffold(topBar = { TopAppBar(title = { Text("Lista de Postos") }) }) { innerPadding ->
    LazyColumn(
      ...
      items(postos) { item ->
        Card(
          ...
          Box(Modifier.fillMaxSize()) { Text(text = item, modifier = Modifier.padding(16.dp)) }
        }
      }
    }
  }
